...
function solution(friends, gifts) {
  // 문제 정리
  // 1. friends는 1차원 문자열 배열 - 중복 없음(객체로도 가능)
  // 2. 주고받은 기록 gifts는 1차원 문자열 배열 - 중복 없음
  // 3. 선물 지수 점수가 제일 높은 사람의 다음달에 받을 선물 갯수를 출력

  // 문제 풀이 로직
  // 1. friends의 각 사람들의 점수를 0점으로 초기화 해준다.
  // 2. 준 사람, 받은 사람을 계산하여 반영한다. 주면 +1 받으면 -1
  // 3. 각 사람들의 선물지수 통합 점수를 Friends_Total_Score에 구한다.
  // 4. 선물 지수에 따른 선물 갯수를 받는 로직을 작성한다.
  ...
  friends.forEach((me) => {
    ...
    friends.forEach((friend) => {
      Friends_Score[me][friend] = 0;
    });
  });

  gifts.forEach((gift) => {
    const [give, receive] = gift.split(" ");
    Friends_Score[give][receive] += 1;
    Friends_Score[receive][give] -= 1;
  });

  const Friends_Total_Score = [];
  friends.forEach((friend) => {
    Friends_Total_Score[friend] = Object.values(Friends_Score[friend]).reduce(
      (acc, cur) => (acc += cur),
      0
    );
  });
  ...
  friends.forEach((me, index) => {
    friends.forEach((friend) => {
      const Give_score = Friends_Score[me][friend];
      const Receive_score = Friends_Score[friend][me];
      if (Give_score > Receive_score) return (result[index] += 1);
      if (
        Give_score === Receive_score &&
        Friends_Total_Score[me] > Friends_Total_Score[friend]
      )
