...
function solution(n, m, section) {
  // 1. 최종적으로 return할 변수 result를 선언한다.
  let result = 0;
  // 2. 처음의 값을 기준 + m -1 만큼 더해서 저장할 목적의 값 purpose를 선언한다.
  let purpose = 0;
  // 3. forEach문으로 purpose와 각 배열의 값을 비교하여
  // purpose가 section의 값보다 작다는 조건에 맞으면 result ++를 한다.
  // purpose는 처음 값부터 페인트를 칠한 영역을 나타낸다.
  section.forEach((section_each) => {
    if (purpose < section_each) {
      purpose = section_each + m - 1;
      result++;
    }
  });
