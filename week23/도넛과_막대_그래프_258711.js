...
function solution(edges) {
  ...
  edges.forEach(([start, end]) => {
    if (!graph[start]) graph[start] = { inN: 0, outN: 0 };
    if (!graph[end]) graph[end] = { inN: 0, outN: 0 };
    graph[start].outN++;
    graph[end].inN++;
  });

  Object.keys(graph).forEach((node_number) => {
    const { inN, outN } = graph[node_number];
    if (inN === 0 && outN > 1) {
      high_point = Number(node_number);
    }
    if (outN === 0) {
      stick_graph += 1;
    }
  ...
  donut_graph = graph[high_point].outN - stick_graph - eight_graph;

  return [high_point, donut_graph, stick_graph, eight_graph];
}
