...
function solution(video_len, pos, op_start, op_end, commands) {
  const timeToSeconds = (time) => {
    let [mm, ss] = time.split(":").map(Number);
    return mm * 60 + ss;
  };
  ...
  let video_len_ss = timeToSeconds(video_len);
  let pos_ss = timeToSeconds(pos);
  let op_start_ss = timeToSeconds(op_start);
  let op_end_ss = timeToSeconds(op_end);
